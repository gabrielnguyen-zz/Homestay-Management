                ...
                <div class="container">
                    <div class="row slider-text align-items-end">
                        <div class="col-md-10 col-sm-12 ftco-animate mb-4">
                            <p class="breadcrumbs"><span class="mr-2"><a href="index.html">Home</a></span> <span>Search Invoice</span></p>
                            <h1 class="mb-3">Search Invoice</h1>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-md-8 ftco-animate">
                        <div class="pt-5 mt-5">
                            <div class="comment-form-wrap pt-5">
                                <h3 class="mb-5">Search Invoice By ID</h3>
                                <input type="text" name="txtSearchInvoice" value="" class="form-control"/><br/>
                                <font color="red">${requestScope.ERROR}</font><br/>
                                <input type="submit" value="Search Invoice" name="action" class="btn btn-primary">
                ...
                <div class="cart_inner">
                    <div class="table-responsive">
                        <table class="table">
                            <thead>
                                <%
                                    List<InvoiceDTO> result = (List<InvoiceDTO>) request.getAttribute("INVOICE");
                                    if (result != null) {
                                        if (result.size() > 0) {
                                %> 
                                <tr>
                            ...
                            <form action="MainController" method="POST">
                                <tr>
                                    ...
                                    <td>
                                        $<%=(dto.getRoomPrice() * dto.getRoomQuantity()) + (dto.getServicePrice() * dto.getServiceQuantity())%>
                                    </td>
                                    <td>
                                        <%= dto.getStatus()%>
                                    </td>
                                    ...
                                    <td>
                                        <%
                                            if (!dto.getStatus().equals("checkout")) {
                                        %>
                                        <form action="MainController">
                                            <input type="hidden" name="txtInvoiceID" value="<%= dto.getInvoiceID()%>" />
                                    ...
                                    <td>
                                        ...
                                        <form action="MainController">
                                            <input type="hidden" name="txtInvoiceID" value="<%= dto.getInvoiceID()%>" />
                                            <input type="hidden" name="txtSearchInvoice" value="<%= request.getParameter("txtSearchInvoice")%>" />
                                            <input class="btn btn-primary" type="submit" value="Checkin" name="action" />
                                        </form>

                                        <% }
                                            if (dto.getStatus().equals("checkin")) {
                                        %>
                                        <form action="MainController">
                                            <input type="hidden" name="txtInvoiceID" value="<%= dto.getInvoiceID()%>" />
                                            <input type="hidden" name="txtSearchInvoice" value="<%= request.getParameter("txtSearchInvoice")%>" />
                                        ...
                                        <%
                                        } else if(dto.getStatus().equals("checkout")){
                                        %>
                                        Check Outed
                                        <%
                                            }%>  
